...
class ShotGenDecoder(nn.Module):
    ...
    def forward(self, input_shot, input_x, input_y, input_player, encode_local_output, encode_global_A, encode_global_B, trg_mask=None, return_attns=False):
        ...
        decode_global_A, _, _, _ = self.global_layer(decode_output_area_A, decode_output_shot_A, encode_global_A, slf_attn_mask=trg_global_A_mask, return_attns=return_attns)
        if decode_output_area_B.shape[1] != 0:
            decode_global_B, _, _, _ = self.global_layer(decode_output_area_B, decode_output_shot_B, encode_global_B, slf_attn_mask=trg_global_B_mask, return_attns=return_attns)
...
class ShotGenEncoder(nn.Module):
    ...
    def forward(self, input_shot, input_x, input_y, input_player, src_mask=None, return_attns=False):
        ...
        encode_global_A, _ = self.global_layer(encode_output_area_A, encode_output_shot_A, slf_attn_mask=src_mask)
        encode_global_B, _ = self.global_layer(encode_output_area_B, encode_output_shot_B, slf_attn_mask=src_mask)
        
        encode_local_output, _ = self.local_layer(encode_output_area, encode_output_shot, slf_attn_mask=src_mask)
